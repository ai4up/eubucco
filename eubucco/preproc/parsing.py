...
def match_attrib_table(gdf, dataset_name, ID, file_path):
    ...
    if dataset_name == 'abruzzo-gov':
        ...
        if dataset_name == 'sardegna-gov':
            ...
            df_attrib = gpd.read_file(os.path.split(file_path)[0]+'/'+os.path.split(file_path)[1][0:8]+'_02_UNITA_VOLUMETRICA.dbf')
        ...
        if dataset_name == 'slovenia-gov':
            ...
            # merge type and link
            df_attrib_type = df_attrib_type.sort_values('NAM_SIF', ascending=True).drop_duplicates('DST_SID').sort_index()
...
def get_stats(df_result_attrib,
    ...
    if dict_val_result['invalid_geom_id']:stats['ID_invalid_geoms'] = [dict_val_result['invalid_geom_id']]
    if dict_val_result['empty_geom_id']:stats['ID_empty_geoms'] = [dict_val_result['empty_geom_id']]
    if dict_val_result['null_geom_id']:stats['ID_null_geoms'] = [dict_val_result['null_geom_id']]    
...
def get_file_paths(dataset_name,path_input_folder,extra,extension):
    ...
    else: file_paths = [f for f in glob.glob(path_input_folder \
            + "/*.{}".format(extension))]
...
def get_extra_attribs(gdf,extra_attrib,extension,var_map,extra,file_path):
    ...
    if type(extra_attrib)==list: return(gdf[['id']+extra_attrib])
    if type(extra_attrib)==dict and extra=='compute_height': return(gdf[['id','max_height']+list(extra_attrib.keys())])
    if extra_attrib==None and extra=='compute_height' and extension=='gml': return(gdf[['id','max_height']])

    elif type(extra_attrib)==dict: return(gdf[['id']+list(extra_attrib.keys())])
    else: sys.exit('Extra attributes are not specified in the correct way (either list or dict).')
...
def has_at_least_one_value(array):
    ...
    '''
    return any(np.not_equal(array, None) == True)
...
def parse_osm_buildings(path_pbf):
    ...
    non_poly_indexes = [i for i,g in enumerate(osm_gdf.geometry) if type(g) not in [shapely.geometry.multipolygon.MultiPolygon, shapely.geometry.polygon.Polygon]]
    if non_poly_indexes != []:
...
def clean_geom(gdf,dataset_name):
    ...
    if any(gdf.geometry.isnull()) or any(gdf.geometry.is_empty):
        ...
        if 'id' in gdf.columns: 
            list_geom_is_empty = list(gdf.id.loc[gdf.geometry.is_empty==True])
            list_geom_is_null = list(gdf.id.loc[gdf.geometry.isnull()==True])
        else: 
            # if no id given save original index before deleting them
            list_geom_is_empty = list(gdf.loc[gdf.geometry.is_empty==True].index)
            list_geom_is_empty = [str(i) for i in list_geom_is_empty]
            list_geom_is_null = list(gdf.loc[gdf.geometry.isnull()==True].index)
...
def parse_tabular(file_path,
    ...
    # edge case brno (only inserted due to the PIK clutser having issues with local crs leading to inf geoms)    
        ...
        gdf = import_csv_w_wkt_to_gdf(file_path,crs=CRS_UNI)
    else: gdf = gpd.read_file(file_path)
    ...
    if dataset_name == 'sicilia-gov': gdf['computed_height'] = gdf.quotagrond - gdf.quotaterra
    ...
    if dataset_name == 'toscana-gov':
        ...
        gdf['q_gronda']=np.where(gdf.q_gronda.notnull(),gdf['q_gronda'].str.replace(',','.').apply(pd.to_numeric),None)
        gdf['q_terra']=np.where(gdf.q_terra.notnull(),gdf['q_terra'].str.replace(',','.').apply(pd.to_numeric),None)
        # compute height from subtracting 'q_terra' from 'q_gronda'  
        gdf['computed_height']=np.where(gdf.q_terra.notnull() & gdf.q_gronda.notnull(),gdf['q_gronda']-gdf['q_terra'], None)

    if dataset_name == 'netherlands-gov': gdf['computed_height'] = gdf.H_DAK_70P - gdf.H_MAAIVELD
    if dataset_name == 'slovenia-gov': gdf['computed_height'] = gdf.H2 - gdf.H3
    ...
    if var_map['id'] is not None: 
        ...
        if dict_val['invalid_geom_id']: dict_val['invalid_geom_id'] = [str(i) for i in dict_val['invalid_geom_id']]
    ...
    if var_map['id'] is not None: gdf = gdf.rename(columns={'id':var_map['id']})
...
def parse_gml(file_path,
    ...
    if socket.gethostname() == '60-MCC': file_path = r'{}'.format(file_path)
    ...
    # check CRS spain
    if dataset_name == 'spain-gov':
        try: local_crs = bldg_elem_list[0].findall(".//gml:Envelope",gml_root.nsmap)[0].attrib['srsName']
        except: print('First building had no CRS')

    # get list of building attributes
    cols = {}
    if var_map['id'] != None: cols['id'] = get_ids(bldg_elem_list,gml_root)
    if var_map['height'] != None: cols['height'] = get_var_attrib(var_map['height'],bldg_elem_list,gml_root,dataset_name)
    
    if var_map['floors'] != None: cols['floors'] = get_var_attrib(var_map['floors'],bldg_elem_list,gml_root,dataset_name)
    
    # edge case: type is given as xlink
    if dataset_name in ['cyprus-gov','czechia-gov','mecklenburg-vorpommern-gov']:
        if dataset_name == 'cyprus-gov': key = '{http://www.opengis.net/gml/3.2}remoteSchema'
        if dataset_name == 'czechia-gov': key = '{http://www.w3.org/1999/xlink}title'
        if dataset_name == 'mecklenburg-vorpommern-gov': key = '{http://www.w3.org/1999/xlink}href'
        cols['type_source'] = get_curr_use_attrib(var_map['type_source'],bldg_elem_list,gml_root,key)
    elif var_map['type_source'] != None: cols['type_source'] = get_uni_attrib(var_map['type_source'],bldg_elem_list,gml_root,dataset_name)
    
    if var_map['age'] != None: cols['age'] = get_uni_attrib(var_map['age'],bldg_elem_list,gml_root,dataset_name)
    
    # get additional building attributes
    if extra_attrib != None:
        for var in extra_attrib.keys():
            ...
            else: sys.exit('Var type is is unknown.')
    ...
    if dataset_name!='hamburg-gov':
        ...
        # for now we keep all the FALSE, as we have to match to an ID, which in some cases comes first in clean_attributes
    ...
    if (extra=='solid' and var_map['height'] == None):
    ...
    if 'id' in gdf.columns: dict_val['invalid_geom_id'] = list(gdf.id.loc[gdf.invalid_geom==True])
    else: 
        # where we don't have it, we save id of original df
        dict_val['invalid_geom_id'] = list(gdf.loc[gdf.invalid_geom==True].index)
    ...
    # edge case: spain
    if dataset_name == 'spain-gov': gdf = add_floors_spain_gml(gdf,file_path,local_crs)

    gdf['geometry'] = [geom.wkt for geom in gdf.geometry]

    if dataset_name == 'spain-gov': return(gdf,count_multipoly,dict_val,local_crs)
    else: return(gdf,count_multipoly,dict_val)
...
def clean_attributes(df,
    ...
    if extension in ['shp','dxf','pbf']:  
        ...
        list_var_source = [var_map[i] for i in var_map.keys() if var_map[i] != None]
        list_var = [i for i in var_map.keys() if var_map[i] != None]
        ...
        # get variables/columns existing in the inputs
        list_var = [i for i in var_map.keys() if i not in ['footprint','wall'] and var_map[i] != None]
    ...
    else: sys.exit('The extension provided is unknown.') 
    ...
    if extra=='compute_height' or (extra=='solid' and var_map['height']==None) or dataset_name=='helsinki-gov': new_cols.remove('height')
    if dataset_name=='spain-gov':new_cols.remove('floors')
    ...
    # separate geometries and attributes (both with id) separately; return id counter for datasets with no id in raw data to create unique id 
...
def parse(path_to_param_file = '/p/projects/eubucco/git-eubucco/database/preprocessing/1-parsing/inputs-parsing.csv',
    ...
    Currently supports the following formats: .gml, .xml, .shp, .dxf, .pbf
    ...
    if arg_as_param != None: param = arg_as_param
    else:
        args = arg_parser(['i'])
        param = args.i
    print(param)
    ...
    if p['extra_attrib'] != None or (p['extra_attrib']==None and p['extra']=='compute_height' and p['extension']=='gml'): 
    ...
    for n,file_path in enumerate(file_paths):
        ...
        if p['extension'] in ['shp','dxf','pbf']: gdf,count_multipoly,dict_val = parse_tabular(file_path,
                                                                                p['dataset_name'],
                                                                                p['var_map']
                                                                                )
        ...
        if ram_percent > max_ram_percent: max_ram_percent = ram_percent
    print("\n========================\n")

    # full stats
    end = time.time() - start
    df_stats = get_stats(df_results_attrib,p['dataset_name'],file_paths,tot_count_multipoly,max_ram_percent,end,dict_val_result)
    print(f'Fault files: {list_faulty_files}')
    if p['dataset_name']=='spain-gov': print(f'CRS: {set(local_crs_files)}')
...
def parse_osm_split(i,
    ...
    df_results_geom.to_csv(os.path.join(path_output,p['country'],'osm',p['dataset_name']+'_'+str(n)+'-3035_geoms.csv'),index=False)
    print('Geometries saved successfully.')

    df_results_attrib.to_csv(os.path.join(path_output,p['country'],'osm',p['dataset_name']+'_'+str(n)+'_attrib.csv'),index=False)
    print('Attributes saved successfully.')

    if p['extra_attrib'] != None or (p['extra_attrib']==None and p['extra']=='compute_height' and p['extension']=='gml'): 
        df_results_extra_attrib.to_csv(os.path.join(path_output,p['country'],'osm',p['dataset_name']+'_'+str(n)+'_extra_attrib.csv'),index=False)
        print('Extra attributes saved successfully.')
