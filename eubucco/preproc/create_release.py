...
def create_lat_lon_release(data_dir: str, output_path: str, tmp_dir: str):
    con = duckdb.connect()
    # Note: Initial installation of spatial extension requires internet connection (-> must be pre-installed on io node)
    con.execute("INSTALL spatial; LOAD spatial;")
    input_pattern = f"{data_dir}/*.parquet"
    ...
    ) TO '{output_path}' (FORMAT PARQUET, COMPRESSION 'ZSTD', ROW_GROUP_SIZE 10000);
    """

    con.execute(query)
    print(f"Success. EUBUCCO release with lat lon created: {output_path}")
