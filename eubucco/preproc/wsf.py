...
def wsf_pixel_counts_country(GADM_w_wsf,
    ...
    for index,row in GADM_w_wsf.iterrows():
        ...
        print(city_name)
        if not city_name is None: pixel_sum_array[index] = wsf_pixel_count_city(GADM_w_wsf,
                                                                                city_name) 
...
def create_wsf_age(gadm_country_code,
    ...
    for file in ['geom','boundary','wsf-evo_geoms','wsf-evo_age']: 
        if left_over==False: paths[file] = get_all_paths(country_name,file)[args.i]
        else: paths[file] = get_all_paths(country_name,file,left_over)[args.i]
    ...
    for path in paths_wsf: wsf = wsf.append(polygonize_wsf(path,boundary,local_crs))
    ...
    get_stats({'city_idx':args.i,'n_bldgs':n_bldgs,'unmatched_bldgs':unmatched_bldgs},end,path_stats,str(args.i),'stat-parts')
...
def create_city_level_ft_area(path_sheet = 'gadm_table.csv',
    ...
    country_name = GADM_sheet.iloc[args.i].country_name
    ...
    for n,path in enumerate(paths):
        ...
        try: tot_ft_area[n] = sum(import_csv_w_wkt_to_gdf(path,3035).geometry.area)
        except: tot_ft_area[n] = 0
