...
def get_paths_dataset(dataset_name,
                     ending,
    ...
    elif gadm_level=='region_name': paths = [path[0] for path in paths if gadm_name == path[1][-3]]

    elif gadm_level=='city_name': paths = [path[0] for path in paths if gadm_name == path[1][-2]]
    ...
    elif gadm_level=='all': paths = [path[0] for path in paths]
...
def metrics_height(df,dict_city,case=None):
    ...
    """
    list_height_attrib = ('height_n','height_p','height_n_inf_0','height_n_0_2','height_n_3_5','height_n_6_10','height_n_11_15','height_n_16_25',
                            'height_n_26_inf','height_n_inf_0','height_p_0_2','height_p_3_5','height_p_6_10','height_p_11_15','height_p_16_25','height_p_26_inf',
                            'height_mean','height_med','height_max','height_min')
...
def metrics_age(df,dict_city, case=None):
    ...
    # if several attrib file could be found (error!)
        ...
        for stat in list_age_attrib:
            dict_city[stat]=np.nan
    # if str in age col
    elif set([type(x) for x in df.age if type(x) == str]): 
        for stat in list_age_attrib:
            dict_city[stat]=np.nan
...
############## 
### MAIN #####
def create_stats_main(db_version=0.1,
    ...
    for path in paths:
        ...
        dict_city, num_bldgs_files,list_ids_files, bid_0 = metrics_extra(dict_city,path_extra, bid_0, num_bldgs_files,list_ids_files)
        ...
        if len([True for lst in list_ids_files if lst==list_id_0])<n_id_files: dict_city['const_id_files']=False
        else: dict_city['const_id_files']=True
    ...
    if len(files_present)>0:
        ...
        df_out.to_csv(os.path.join(path_overview,p['country']+'_overview_'+str(len(files_present))+'.csv'),index=False)
    else: df_out.to_csv(os.path.join(path_overview,p['country']+'_overview.csv'),index=False)
