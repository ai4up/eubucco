...
def reproj_to_3035_in_db(country_names = ['france','netherlands'],
    ...
    for country_name in country_names:
        ...
        for path in get_all_paths(country_name,'geom') + get_all_paths(country_name,'buffer'):
            ...
            try: save_csv_wkt(import_csv_w_wkt_to_gdf(path,local_crs[country_name]).to_crs(3035),path)
        ...
        for element in failed: textfile.write(element + "\n")
        textfile.close()
...
def reproject(crs_uni = 'EPSG:3035',
    ...
    ## read in data
    gdf = import_csv_w_wkt_to_gdf(os.path.join(path_root,p['country'],p['dataset_name']+'_geom.csv'),p['local_crs'])
...
def reproject_osm(crs_uni = 'EPSG:3035',
            path_root = '/p/projects/eubucco/data/1-intermediary-outputs/',
            path_to_param_file = '/p/projects/eubucco/git-eubucco/database/preprocessing/1-parsing/inputs-parsing.csv',
            path_to_stats_file = '/p/projects/eubucco/git-eubucco/database/preprocessing/1-parsing/stats/reproj/failed_reproj.csv'):
...
def reproject_osm_split(crs_uni = 'EPSG:3035',
            path_root = '/p/projects/eubucco/data/1-intermediary-outputs/',
            path_to_param_file = '/p/projects/eubucco/git-eubucco/database/preprocessing/1-parsing/inputs-parsing.csv'):
    ...
    if p['country']=='germany': num_files = [1,3,8,15,16,20,22]
    else: num_files = range(dict_file_num[p['country']])    

    for k in num_files:
        ## read in data
        try: 
            gdf = import_csv_w_wkt_to_gdf(os.path.join(path_root,p['country'],'osm',p['country']+'-osm_'+str(k)+'_geom.csv'),p['local_crs'])
            ...
            # saving to 1-intermediary-outputs
            gdf.to_csv(os.path.join(path_root,p['country'],'osm',p['country']+'-osm_'+str(k)+'3035_geoms.csv'),index=False)
            print('saved {}/{} reproj geom successfully'.format(k,num_files))
        except: print('skipped {}/{} geom '.format(k,len(num_files)))